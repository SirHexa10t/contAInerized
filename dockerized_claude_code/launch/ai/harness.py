"""The HARNESS — the agent CLI a container runs for an AI — as a record of the
facts the launcher needs about it. This is the DATA half of the adapter
`plans/harness_coupling.md` (shape A) and `plans/harness_commonality.md`
describe: everything that differs between harnesses only by NAME (binary,
flags, filenames, env vars, hosts) is a field here, so the rest of the tree
reads `active_harness().<field>` instead of spelling Claude Code's word. The
BEHAVIOUR half — rendering settings and policies, parsing transcripts and the
event stream, composing the persona — differs by STRUCTURE and arrives as
methods when those seams are generalised (`adding_an_ai.md`, order of work).

One instance per harness lives in its own module (`claude_code.py` today); the
registry and `active_harness()` are in `launch/ai/__init__.py`. Fields carry
only what has a caller in the tree today — the fuller questionnaire is the
commonality matrix — and a harness that lacks one is a `None`, never a guess.
"""
...
class Harness:
    ai_key: str                      # the `agents/ai/<key>` member this CLI runs (the tag carries the display facts: vendor, colours, steps)
    name: str                        # the CLI's own name as the vendor writes it — "Claude Code", "Gemini CLI", "Codex CLI", "Grok Build" — for the strings a user reads
