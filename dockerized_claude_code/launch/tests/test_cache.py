...
class _CacheTestBase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._patches = [
            patch.object(agent_composition, "CACHE_MOUNTS", self.fake_mounts),
            patch.object(agent_composition, "CACHE_ROOT", self.host_root),
            patch.object(agent_composition, "any_agent_container_running", return_value=False),
...
class TestPrepareCaches(_CacheTestBase):
    def test_creates_missing_dirs(self):
        ...
        agent_composition.prepare_caches()
...
class TestPruneCaches(_CacheTestBase):
    ...
    def setUp(self):
        ...
        self._threshold_patch = patch.object(agent_composition, "CACHE_PRUNE_THRESHOLD_GB", 0.000_001)   # 1KB
        self._age_patch = patch.object(agent_composition, "CACHE_PRUNE_MIN_AGE_DAYS", 7)
    ...
    def test_skips_when_agent_container_running(self):
        ...
        with patch.object(agent_composition, "any_agent_container_running", return_value=True):
            agent_composition.prune_caches()
    ...
    def test_under_threshold_not_pruned_even_when_old(self):
        ...
        with patch.object(agent_composition, "CACHE_PRUNE_THRESHOLD_GB", 100):   # 100 GB
    ...
    def test_missing_cache_dir_is_skipped(self):
        ...
        self.assertFalse(self.cache_a.exists())
        self.assertFalse(self.cache_b.exists())
        agent_composition.prune_caches()   # would raise if it tried to walk a missing dir
...
class TestCacheLifecycleScenario(_CacheTestBase):
    ...
    def setUp(self):
        super().setUp()
        self._threshold_patch = patch.object(agent_composition, "CACHE_PRUNE_THRESHOLD_GB", 0.000_001)
        self._age_patch = patch.object(agent_composition, "CACHE_PRUNE_MIN_AGE_DAYS", 7)
        self._threshold_patch.start()
        self._age_patch.start()
        self.addCleanup(self._threshold_patch.stop)
