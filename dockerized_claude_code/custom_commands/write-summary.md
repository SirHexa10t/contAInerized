...
Frame the whole summary under a `## Project Summary` top-level heading, with each section below as `###`. Immediately under that heading, include a one-line italicized disclaimer covering four facts: provenance (AI-generated by `/write-summary`), purpose (a summary/description of the current project), audience (other AI sessions that need to catch up on the project without re-analyzing it line-by-line), and a freshness caveat (may be out of date). The audience note orients future AI readers; the provenance and staleness notes protect any human who greps the repo from mistaking the file for hand-maintained documentation.
