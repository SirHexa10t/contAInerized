...
- **Do not** write user-, developer-, or runtime-specific content into the summary under any heading. The summary describes the **project**, which is the same regardless of who's running it or where. Specifically excluded:
  ...
  - **"This dev shell" details** — which CLIs / tools happen to be installed in the current agent's container, which agents the operator has configured on their host, what `/home/<user>/` looks like for them, environment variables in the current shell. Section headings like "External tooling available in this dev shell" are a smoking gun — delete and rewrite.
  - **Per-instance / per-session state** — which mode flags the current session was launched with, which workspace is mounted right now, what any agent's MEMORY.md currently contains.

  Rule of thumb: *if two different teams cloned this repo and each ran `/write-summary`, would they get the same answer?* If not, the content is runtime-specific and belongs nowhere in the file. Project-level facts (dependencies declared in `package.json` / `pyproject.toml`, tools listed in the Dockerfile, build infra in the README) ARE part of the project; what one operator happens to have installed locally is not.
