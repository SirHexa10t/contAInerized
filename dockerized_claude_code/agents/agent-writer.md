...
- `agents/engine/*/tag.info` + `tag.budget` — the engines (capability-standard + switches personalities, in the launcher's own words: `standard = "2025Q4"`, `memory = false`, `max_output_tokens = 36000`; a standard is `cheapest`, `best`, or a dated quarter from `agents/ai/capability.standards`). Nested dirs inherit and override their parent's `tag.budget`. Read `tag.info` for the engine's intent (it names no model) and `agents/ai/<key>/efforts.tiers` for the model an AI runs to meet that standard; the picker renders that model beside the engine's words.
- `agents/ai/*/` and `agents/harness/*/tag.info` — the AIs (`⟪Claude⟫` …) and the agent CLIs that wrap them (`⟦ClaudeCode⟧` …). A `.lego` may pre-pick `ai` and `harness`; the harness must be one that runs the AI (`ais` in its tag.info), and an unset one means the AI's default. Neither shows on an agent's picker row — they are decided per instance.
...
4. **Write the `.lego`.** Always `ai = "..."` and `harness = "..."` — every shipped sibling pins them, so the tree passes its own audit (`python -m launch.audit` reports `implicit_ai` / `implicit_harness` for a `.lego` that names neither) and stays put if the default AI ever moves; the harness must be one that runs the AI. Then only the axes that differ from nothing: `engine = "..."`, `professions = [...]`, `specialties = [...]`, `policies = [...]`. List a nested tag's ancestors explicitly (`["code", "webdev"]`, not `["webdev"]`). Hyphenate multi-word names — every sibling does.
